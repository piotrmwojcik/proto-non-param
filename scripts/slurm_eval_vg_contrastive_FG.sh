#!/bin/bash
# Zero-shot RIS evaluation for runs F and G only.
#
# Submits 14 SLURM jobs: 2 variants (F/G) × 7 dataset/split combos
#   Gref : val
#   unc  : val, testA, testB
#   unc+ : val, testA, testB
#
# Variants:
#   F — k=5, dedup, hard positive mining, hard InfoNCE (30 epochs)
#   G — k=5, dedup, hard positive mining, soft negatives tau_label=0.5 (30 epochs)
#
# Results land in:
#   eval_results/vg_contrastive/contr_F/pnp_refer/{dataset}_{split}.json
#   eval_results/vg_contrastive/contr_G/pnp_refer/{dataset}_{split}.json
#
...
#   CONTR_BASE   Base dir containing run_F_* / run_G_* subdirs
#   DATA_ROOT    Path to refcoco/ directory
#
# Usage:
#   bash scripts/slurm_eval_vg_contrastive_FG.sh
...
declare -A CKPTS=(
  [F]="${CONTR_BASE}/run_F_contrastive10_k5_hardmining_30ep/ckpt.pth"
  [G]="${CONTR_BASE}/run_G_contrastive10_k5_hardmining_soft05_30ep/ckpt.pth"
)

declare -A LABELS=(
  [F]="k=5, dedup, hard mining, hard InfoNCE (30 epochs)"
  [G]="k=5, dedup, hard mining, soft negatives tau=0.5 (30 epochs)"
)
...
echo "=== PNP Contrastive VG — Zero-shot RIS Evaluation (runs F/G, 30 epochs) ==="
echo "  Ckpt F : ${CKPTS[F]}"
echo "  Ckpt G : ${CKPTS[G]}"
echo "  Data   : ${DATA_ROOT}"
echo "  Results: eval_results/vg_contrastive/contr_{F,G}/pnp_refer/"
echo ""

for VARIANT in F G; do
  CKPT="${CKPTS[$VARIANT]}"
  if [ ! -f "${CKPT}" ]; then

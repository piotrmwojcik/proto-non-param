#!/bin/bash
# 2x2 ablation: (KL vs JSD) x (frozen residual vs trained+constrained residual) on Visual Genome.
#
...
#
# All runs use --target-mode topk with top-k-concepts=5 (VG region descriptions are shorter).
# topk is required for JSD to have clean zeros providing the negative signal.
#
# Prerequisite: VG vocabulary cache must exist at ${VOCAB_DIR}/vg_cache.pt.
# Build it first with: bash scripts/slurm_build_vg_vocab.sh
#
...
#
# Usage:
#   bash scripts/slurm_train_vg_ablation.sh
#   SCRATCH=/my/scratch bash scripts/slurm_train_vg_ablation.sh
...
LOG_DIR="${LOG_DIR:-${SCRATCH}/train_logs/vg_ablation}"
...
echo "=== Visual Genome Residual × Loss Ablation ==="
...
JOB_A=$(sbatch --parsable \
  ${SLURM_COMMON} \
  --job-name=vg-A-kl-base \
  --output="${LOG_SLURM}/vg_abl_A_%j.out" \
  --error="${LOG_SLURM}/vg_abl_A_%j.err" \
  --wrap="${WRAP_HEADER}
python train.py \\
  ${TRAIN_COMMON} \\
  --loss-type kl \\
...
JOB_B=$(sbatch --parsable \
  ${SLURM_COMMON} \
  --job-name=vg-B-kl-res \
  --output="${LOG_SLURM}/vg_abl_B_%j.out" \
  --error="${LOG_SLURM}/vg_abl_B_%j.err" \
  --wrap="${WRAP_HEADER}
python train.py \\
  ${TRAIN_COMMON} \\
  --loss-type kl \\
...
JOB_C=$(sbatch --parsable \
  ${SLURM_COMMON} \
  --job-name=vg-C-jsd-base \
  --output="${LOG_SLURM}/vg_abl_C_%j.out" \
  --error="${LOG_SLURM}/vg_abl_C_%j.err" \
  --wrap="${WRAP_HEADER}
python train.py \\
  ${TRAIN_COMMON} \\
  --loss-type jsd \\
...
JOB_D=$(sbatch --parsable \
  ${SLURM_COMMON} \
  --job-name=vg-D-jsd-res \
  --output="${LOG_SLURM}/vg_abl_D_%j.out" \
  --error="${LOG_SLURM}/vg_abl_D_%j.err" \
  --wrap="${WRAP_HEADER}
python train.py \\
  ${TRAIN_COMMON} \\
  --loss-type jsd \\

#!/bin/bash
#SBATCH --job-name=train_caption_filter
...
#SBATCH --output=/net/tscratch/people/plgabedychaj/logs/train_caption_filter_%j.out
#SBATCH --error=/net/tscratch/people/plgabedychaj/logs/train_caption_filter_%j.err
...
# Caption-constrained CLIP scoring experiment.
# Identical to run_train_coco_vg_clip_scores.sh but with --clip-scores-caption-filter:
# before top-50 selection, vocab words not present in the image's captions are masked
# out, preventing synonyms (e.g. "feline") from filling slots when the caption says "cat".
...
python train.py \
  ...
  --clip-scores-temperature 0.01 \
  --clip-scores-top-k 50 \
  --clip-scores-caption-filter \
  --log-dir /net/tscratch/people/plgabedychaj/train_logs/coco_vg_caption_filter \
